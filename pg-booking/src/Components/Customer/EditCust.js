...
class EditCust extends Component {
  ...
  componentDidMount() {
    var id = this.props.location.pathname.substr(10)
    axios.get('http://localhost:4000/customers/EditUser/' + id)
      .then((res) => {
        this.setState({
  ...
  onSubmit(e) {
    ...
    var id = this.props.location.pathname.substr(10)
    axios.put('http://localhost:4000/customers/UpdateUser/' + id, UserObj)
      .then((res) => {
